...
DROP POLICY IF EXISTS "Users manage own personal tasks" ON public.tasks;

CREATE POLICY "Users manage own personal tasks" ON public.tasks
FOR ALL TO authenticated
...
DROP POLICY IF EXISTS "Personal tasks restricted to creator" ON public.tasks;

CREATE POLICY "Personal tasks restricted to creator" ON public.tasks
AS RESTRICTIVE FOR ALL TO authenticated
USING (project_id IS NOT NULL OR created_by = auth.uid())
WITH CHECK (project_id IS NOT NULL OR created_by = auth.uid());
